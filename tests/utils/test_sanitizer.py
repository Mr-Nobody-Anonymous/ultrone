...
def _sanitizer_or_skip(**kwargs):
    """Build a DataSanitizer, raising SkipTest when presidio/spacy are absent."""
    try:
        from ultron.utils.sanitizer import DataSanitizer

        return DataSanitizer(**kwargs)
    except (ImportError, ModuleNotFoundError, RuntimeError) as e:
        raise unittest.SkipTest(f"PII sanitizer dependencies unavailable: {e}")
...
class TestDataSanitizerPII(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.sanitizer = _sanitizer_or_skip()
...
class TestDataSanitizerCustomPatterns(unittest.TestCase):
    ...
    def test_custom_pattern_applied(self):
        ...
        sanitizer = _sanitizer_or_skip(custom_patterns=[(r"PROJ-\d+", "<TICKET>")])
