...
def _mock_sanitizer():
    """Sanitizer stub so tests don't require presidio/spacy models."""
    sanitizer = MagicMock()
...
class TestMemoryServiceSearch(unittest.TestCase):
    ...
    def test_search_sorts_by_tier_boosted_score_desc(self):
        ...
        svc = MemoryService(db, emb, sanitizer=_mock_sanitizer(), config=cfg)
    ...
    def test_search_rejects_detail_level_full(self):
        svc = MemoryService(MagicMock(), MagicMock(), sanitizer=_mock_sanitizer(), config=UltronConfig())
...
class TestMemoryServiceSummariesAndRebalance(unittest.TestCase):
    ...
    def test_generate_summaries_rule_fallback_first_line_l0(self):
        svc = MemoryService(MagicMock(), MagicMock(), sanitizer=_mock_sanitizer(), llm_service=None)
    ...
    def test_tier_rebalance_distributes_by_percentile(self):
        ...
        cfg.cold_ttl_days = 7
        svc = MemoryService(db, MagicMock(), sanitizer=_mock_sanitizer(), config=cfg)
...
class TestMemoryServiceDelegates(unittest.TestCase):
    ...
    def test_get_memory_stats_delegates(self):
        ...
        svc = MemoryService(db, MagicMock(), sanitizer=_mock_sanitizer())
