...
def _default_research_db_dir() -> str:
    """Resolve the ``research_db/`` package directory (monorepo-safe)."""
    import research_db
    return str(Path(research_db.__file__).resolve().parent)
...
def persist_run_metadata(
    ...
    store = JSONResearchStore(
        base_dir=str(base_dir) if base_dir is not None else _default_research_db_dir()
    )
    ids: List[str] = []
