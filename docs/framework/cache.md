...
> [!TIP]
>
> Location within the framework `Adelise-agent-framework/cache`.
...
```ts
import { UnconstrainedCache } from "Adelise-agent-framework/cache/unconstrainedCache";
...
```ts
import { SlidingCache } from "Adelise-agent-framework/cache/slidingCache";
import { WikipediaTool } from "Adelise-agent-framework/tools/search/wikipedia";
...
import { BaseMessage } from "Adelise-agent-framework/llms/primitives/message";
...
```ts
import { FileCache } from "Adelise-agent-framework/cache/fileCache";
import * as os from "node:os";

const cache = new FileCache({
  fullPath: `${os.tmpdir()}/Adelise_file_cache_${Date.now()}.json`,
});
...
const fileCache = await FileCache.fromProvider(memoryCache, {
  fullPath: `${os.tmpdir()}/Adelise_file_cache.json`,
});
...
```ts
import { Cache } from "Adelise-agent-framework/cache/decoratorCache";
...
```ts
import { Cache, SingletonCacheKeyFn } from "Adelise-agent-framework/cache/decoratorCache";
...
```ts
import { CacheFn } from "Adelise-agent-framework/cache/decoratorCache";
...
```ts
import { BaseCache } from "Adelise-agent-framework/cache/base";
import { NotImplementedError } from "Adelise-agent-framework/errors";
