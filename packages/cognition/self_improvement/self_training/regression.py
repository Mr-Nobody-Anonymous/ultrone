...
def _scenario(seed: int, *, prefix: str, **overrides) -> TaskProfile:
    ...
    return TaskProfile(
        ...
        reasoning_depth=overrides.get("reasoning_depth", base.reasoning_depth),
        context_requirement=overrides.get(
            "context_requirement", base.context_requirement
        ),
        tool_requirement=overrides.get("tool_requirement", base.tool_requirement),
        latency_sensitivity=overrides.get(
...
def build_families(
    n_each: int = 8, base_seed: int = 400
) -> Dict[str, List[TaskProfile]]:
    """Deterministic task families in disjoint seed zones."""
    children = {}
    children["normal"] = [_scenario(11 + i, prefix="normal") for i in range(n_each)]
    children["unseen"] = [
        _scenario(
            base_seed + i,
            prefix="unseen",
    ...
    children["difficult"] = [
        _scenario(201 + i, prefix="difficult", difficulty=0.9, reasoning_depth=0.9)
        for i in range(n_each)
    ]
...
def score_weights(
    ...
    *,
    ...
    orchestrator = Orchestrator(policy or RoutingPolicy(), executor=executor)
...
class RegressionReport:
    ...
    def family_regressions(self) -> List[str]:
        return [name for name, rep in sorted(self.families.items()) if rep.regressions]
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "families": {
                name: self.families[name].__dict__ for name in sorted(self.families)
            },
            "passed": self.passed,
        }
...
class RegressionSuite:
    ...
    def run(
        self, candidate: LearnedWeights, baseline: LearnedWeights
    ) -> RegressionReport:
        ...
        for name, profiles in sorted(self.families.items()):
            ...
            base_mean = (
                sum(base_scores.values()) / len(base_scores) if base_scores else 0.0
            )
            cand_mean = (
                sum(cand_scores.values()) / len(cand_scores) if cand_scores else 0.0
            )
