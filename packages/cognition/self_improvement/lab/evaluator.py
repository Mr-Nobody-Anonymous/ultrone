...
def _chain_registry() -> SkillRegistry:
    ...
    for L in range(1, 7):  # L+1 skills needed incl. goal
        ...
        for i in range(1, L):
            nxt = f"c{L}_{i}"
            reg.register(Skill(f"c{L}_step{i}", "synthetic", frozenset({prev}), nxt))
            prev = nxt
        reg.register(Skill(f"c{L}_final", "synthetic", frozenset({prev}), f"c{L}_goal"))
    return reg
...
def _memory_score(capacity: int) -> float:
    ...
    report = run_continual_sequence(tasks, n_train_reps=12, capacities=(capacity,))
...
def _tool_score(policy: str) -> float:
    ...
    box.register(
        Tool("top_of_set", "set", "insight", lambda s: sorted(s)[0] if s else "")
    )
...
def _machine_control_score(seed: int = 7) -> float:
    ...
    tasks = report["tasks"]
    settled_fraction = sum(1 for t in tasks.values() if t["settled"]) / len(tasks)
...
class CapabilitySnapshot:
    ...
    def efficiency(self) -> float:
        ...
        return round(
            self.capability_index / max(1.0, math.log10(params_m)),
            6,
        )
...
def measure_genome(genome: Genome, seed: int = 0) -> CapabilitySnapshot:
    ...
    perception = run_perception_suite(seed=seed + 5, n_trials=120)["fusion_accuracy"]
    ...
    raw["mathematics"] = round(0.5 * raw["prediction"] + 0.5 * raw["reasoning"], 4)
    raw["language"] = round(
        0.4 * raw["perception"] + 0.3 * raw["memory"] + 0.3 * raw["tool_use"], 4
    )
    ...
    latency_proxy = round(
        5.0
        + genome.planning_depth * 8.0
        + (genome.parameter_count / 1e8) * 3.0
        + (2.0 if genome.tool_policy == "deep" else 0.0),
        3,
    )
