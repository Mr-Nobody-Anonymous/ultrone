...
def _find_repo_root(start: Path) -> Path:
    """Walk up to the directory containing pyproject.toml (repo root)."""
    node = start.resolve()
    while True:
...
# Monorepo note: this vendored tree lives under vendor/original_source/, so a
# fixed parent-walk lands inside vendor/. Walk up to the real repo root, and
# note showcase docs live under docs/ultron/<lang>/Showcase.
_REPO_ROOT = _find_repo_root(Path(__file__))
_SHOWCASE_BY_LANG = {
    "zh": _REPO_ROOT / "docs" / "ultron" / "zh" / "Showcase",
    "en": _REPO_ROOT / "docs" / "ultron" / "en" / "Showcase",
}
