...
export function EntityInfoCard() {
    ...
    const {heading} = hoveredEntity;
    const headingDisplay = heading !== undefined ? `${heading.toFixed(0)}°` : null;

    const typeDisplay = hoveredEntity.properties?.type !== undefined ? String(hoveredEntity.properties.type) : null;

    const casualtiesDisplay = hoveredEntity.properties?.casualties !== undefined ? String(hoveredEntity.properties.casualties) : null;
          ...
          {isConflictZone && escalationScore !== undefined && (
            <div className="entity-info-card__prop">
              ...
              <span className="entity-info-card__prop-value" style={{ color: escalationScore >= 5 ? "#991b1b" : escalationScore >= 4 ? "#ef4444" : escalationScore >= 3 ? "#f97316" : "#fbbf24" }}>
                {SEVERITY_LABELS[escalationScore] || escalationScore}
                {' '}
                / 5
              </span>
            </div>
