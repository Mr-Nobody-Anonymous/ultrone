#!/usr/bin/env node
/**
 * URL Linter: flags hardcoded production domain references that should be env-overridable.
 *
 * Flags any source line containing a production domain (e.g. "worldwideview.dev")
 * UNLESS the line also contains an env-var read (process.env), a fallback operator
 * (|| or ??), or an explicit suppression comment (// lint-url: allow).
 *
 * Usage: node scripts/lint-urls.mjs [--domain <domain>]
...
const LINE_ALLOW = [/process\.env/, /\|\|/, /\?\?/, /\/\/\s*lint-url:\s*allow/i, /^\s*\/\//, /^\s*\*/]
