...
import { createInfrastructureLayers } from 'ultrone-globe/infrastructure';
import { createLocalGeoJsonLayer } from 'ultrone-globe/infrastructure/geojson';
...
test('package exports import without an application, DOM, fetch, or timers', () => {
  const result = spawnSync(
    ...
    await import('ultrone-globe/infrastructure');
    await import('ultrone-globe/infrastructure/geojson');
    await import('ultrone-globe/infrastructure/lod');
  `,
    ],
...
test('consumer build includes only infrastructure code and resolves assets under a non-root base', async () => {
  ...
  const result = await build({
    ...
    build: {
      ...
      rollupOptions: {
        input: fileURLToPath(
          import.meta.resolve('ultrone-globe/infrastructure'),
        ),
        external: ['cesium'],
